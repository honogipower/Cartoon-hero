  ...
  var firePixel = function (advertiser, data_object) {
    ...
    if (isUniversal) {
      ...
      if (!data_object.category) {
        consoleLog("WARN: Universel Pixel fire: Category not specified. Pixel ID: " + advertiser.flip_id);
        if (!data_object.subcategory){
          consoleLog("WARN: Universel Pixel fire: Subcategory not specified. Pixel ID: " + advertiser.flip_id);
        }
      }
    ...
    post_data.timestamp = new Date().getTime(), 
